...
class ValueInterpretation {
	 ...
	 * @param interpretationTemplate A MathML template that can contains the identifiers from patternstr, e.g. <apply><csymbol>times</csymbol><ci>#1</ci><ci>1000000</ci></apply>
	 */
	...
	public String getValueInterpretation(String value) {	
		...
		if (matcher.matches()) {
			for (int i = 1; i <= matcher.groupCount(); i++) {
				interpretationTemplate = interpretationTemplate.replaceAll("#"+i, matcher.group(i));
			}
